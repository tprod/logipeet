...
CREATE TABLE IF NOT EXISTS `mydb`.`Cannil` (
  ...
  `Annimals_idAnnimals` INT NOT NULL,
  `Sensors_idSensors` INT NOT NULL,
  `In_Out_Can_id_inout_c` INT NOT NULL,
  PRIMARY KEY (`idCannil`, `Annimals_idAnnimals`, `Sensors_idSensors`, `In_Out_Can_id_inout_c`),
  INDEX `fk_Cannil_Annimals1_idx` (`Annimals_idAnnimals` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_Cannil_Annimals1`
    FOREIGN KEY (`Annimals_idAnnimals`)
    REFERENCES `mydb`.`Annimals` (`idAnnimals`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
